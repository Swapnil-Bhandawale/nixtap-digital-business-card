const fs = require('fs');
const content = `import { useEffect, useState, useCallback, useMemo } from 'react';
          ...
          <div className="max-w-full overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                ...
                {status === 'ready' && filteredLeads.map((lead) => (
          ...
          {status === 'ready' && filteredLeads.length > 0 && (
            <div className="border-t border-slate-100 dark:border-slate-800/50 p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Showing <span className="font-medium text-slate-800 dark:text-white">{filteredLeads.length}</span> leads
              </p>
            </div>
          )}
        </div>
...
`;
fs.writeFileSync('src/pages/app/Leads.jsx', content, 'utf8');
